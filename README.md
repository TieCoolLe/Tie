...
| 平台 | 文件 |
|------|------|
| Linux x86_64 | `tiecode` |
| Windows x86_64 | `tiecode.exe` |
...
在tiecode gui主界面，从左侧agents列表点击选择一个agent，在主界面的信息输入框下面选择大模型的供应商和模型，在信息输入框中描述想让agent处理的工作内容，回车发送。

信息输入框中下方的设置栏目还包括：工作目录、agent权限、大模型Temp参数、Plan模式和思考强度。
